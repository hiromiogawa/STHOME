...
const minmaxDecision = (width: number | keyof DeviceType, minmax: 'min' | 'max'): number => {
...
// px→vw
export const vw = (px: number, width: number | keyof DeviceType = Device.tb, minmax: 'min' | 'max' = 'min'): string => {
    const _width = minmaxDecision(width, minmax)
    return `${(( 100 / _width) * px)}vw`
}

// MediaQuery
export const media = (width: number | keyof DeviceType = Device.tb, minmax: 'min' | 'max' = 'min'): string => {
